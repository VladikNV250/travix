...
export const useStopFormViewModel = ({
    ...
    const editMode = useMemo(() => !!initialData, [initialData]);
    ...
    const onDataChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const onSaveStop = useCallback(async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
    ...
    }, [dispatch, editMode, formData, initialData, map, onClose, tripId, isFormValid])
