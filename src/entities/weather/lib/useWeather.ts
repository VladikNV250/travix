...
export const useWeather = () => {
    ...
    const getWeather = async (latlng: string, date: Date) => {
        ...
        try {
            ...
            if (date.getTime() - today.getTime() >= 0 && date.getTime() - today.getTime() <= day) {
                setWeather(await getWeatherToday(latlng))
            } else if (date.getTime() - today.getTime() >= 0 && date.getTime() - today.getTime() <= 14*day) {
                setWeather(await getWeatherForecast(latlng, date));
            } else if (date.getTime() - today.getTime() > 14*day && date.getTime() - today.getTime() <= 300*day) {
